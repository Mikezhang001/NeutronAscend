 ...
 private:
     ...
     uint32_t n_divide_16 = 0; // N除以16的结果，用于分组
     uint32_t ratio = 0; // 任务分配比例，默认1:1
 ...
 public:
     ...
     {   
        ...
        //  printf("GetBlockIdx =  %d, GetBlockNum =  %d\n", this->process_id, this->process_num);

        //  // 在分离架构融合算子场景，可以查看Cube/Vector的配比
        //  this->ratio = GetTaskRation();
        //  AscendC::PRINTF("task ration is %u\n", this->ratio);

        //  //获取AI Core上Vector核的数量
        //  int64_t subBlockNum = GetSubBlockNum();
        //  int64_t subBlockID = GetSubBlockIdx();
        //  PRINTF("sub block num is %d\n", subBlockNum);
        //  PRINTF("sub block id is %d\n", subBlockID);
        //  printf("----------------------------------------------------------\n");
     ...
     {
         // 组分给aicore
         if(process_id < this->n_divide_16)
         {
            for (int group = this->process_id; group < this->group_num; group += this->n_divide_16)
            {
 ...
 private:
     __aicore__ inline void InitData(int M, int N, int K)
     {
         ...
         this->n_divide_16 = N / 16;
         group_count = this->n_divide_16;


         block_sum = GetBlockNum();//总共多少个aicore
         group_size = 1; // 每组aicore的数目
         ...
         this->L2_m = this->L2_m / 16 * 16;                                                                  //(确保16的倍数)   
...
extern "C" __global__ __aicore__ void mmad_custom_tp(GM_ADDR x, GM_ADDR y, GM_ADDR z, GM_ADDR workspace, GM_ADDR tiling) {
